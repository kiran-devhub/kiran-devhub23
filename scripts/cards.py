...
These are files in your own repo, so they render as long as GitHub renders.
    ...
    python scripts/cards.py --user kiran-devhub --out assets

Writes <out>/card-stats-{dark,light}.svg plus one card per repo listed in
assets/projects.json, or up to four recent public repos when the list is empty.
...
THEMES = {
    "dark": {
        "bg": "#0d1117", "border": "#30363d", "title": "#aa9bef",
...
def main(argv=None):
    ...
    if not wanted:
        recent = [r for r in repos if not r.get("fork") and not r.get("archived") and not r.get("private")]
        recent.sort(key=lambda r: r.get("pushed_at") or "", reverse=True)
        wanted = [{"repo": r["name"]} for r in recent[:4]]
